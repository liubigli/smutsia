...
def minimum_spanning_tree(csgraph, additive=False, return_hierarchy=False, overwrite=False):
    r"""
    minimum_spanning_tree(csgraph, overwrite=False)
    Return a minimum spanning hierarchy of an undirected graph
    A minimum spanning hierarchy is a graph consisting of the subset of edges
    ...
    csgraph : array_like or sparse matrix, 2 dimensions
        ...
        (see notes below).
    additive : additive version of the minimum spanning hierarchy algorithm
        It returns not only the graph but also the vector of
    ...
    span_tree : csr matrix
        ...
        spanning hierarchy over the input (see notes below).
    Notes
    -----
    ...
    Examples
    --------
    The following example shows the computation of a minimum spanning hierarchy
    over a simple four-component graph::
         input graph             minimum spanning hierarchy
             (0)                         (0)
            /   \                       /
           3     8                     3
          /       \                   /
        (3)---5---(1)               (3)---5---(1)
          \       /                           /
           6     2                           2
            \   /                           /
             (2)                         (2)
    It is easy to see from inspection that the minimum spanning hierarchy involves
    ...
    valnodes = np.ones(N, dtype=ITYPE)

    countout = np.zeros([N-1,2], dtype=ITYPE)

    min_area_weights = np.zeros_like(data)
    ...
    if additive:
        sp_tree = csr_matrix((min_area_weights, indices, indptr), (N, N))
    else:
        sp_tree = csr_matrix((data, indices, indptr), (N, N))
    sp_tree.eliminate_zeros()
    if additive:
        return sp_tree, countout
...
cdef void _min_spanning_tree(DTYPE_t[::1] data,
                             ...
                             ITYPE_t[::1] valnodes,
                             ITYPE_t[:,::1] countout,
                             DTYPE_t[::1] min_area_weights) nogil:
    # Work-horse routine for computing minimum spanning hierarchy using
    #  Kruskal's algorithm.  By separating this code here, we get more
    #  efficient indexing.
    cdef unsigned int i, j, V1, V2, R1, R2, n_edges_in_mst, n_verts, n_data, value
    cdef DTYPE_t weight
    ...
    while i < n_data and n_edges_in_mst < n_verts - 1:
        ...
        # in the spanning hierarchy.
        if R1 != R2:

            value = valnodes[R1] + valnodes[R2]
            ...
            if rank[R1] > rank[R2]:
                ...
                predecessors[R2] = R1
                rank[R1] += 1
            countout[n_edges_in_mst, 0] = valnodes[R1]
            countout[n_edges_in_mst, 1] = valnodes[R2]
            min_area_weights[j] = data[j] * min(valnodes[R1], valnodes[R2])
            valnodes[R1] = value
            valnodes[R2] = value

            # updating the number of edges in the mst
            n_edges_in_mst += 1
        else:
            data[j] = 0
