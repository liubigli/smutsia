...
python ground_detection.py --dataset=$DATASET --method='cnn' --params=$PARAMDIR'unet.yaml' --sequence=$SEQUENCE --start=$START --end=$END --step=$STEP --chunk_size=$CHUNK_SIZE
python ground_detection.py --dataset=$DATASET --method='cnn' --params=$PARAMDIR'unet_normals.yaml' --sequence=$SEQUENCE --start=$START --end=$END --step=$STEP --chunk_size=$CHUNK_SIZE